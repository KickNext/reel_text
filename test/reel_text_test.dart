...
void main() {
  ...
  testWidgets('mixed bidi Latin runs keep visual left-to-right order', (
    tester,
  ) async {
    const reelKey = ValueKey('reel_bidi_visual_order');
    const text = 'Gate A12 שער';
    const style = TextStyle(fontSize: 32, fontWeight: FontWeight.w700);
    ...
    final g = tester.getRect(find.text('G'));
    final a = tester.getRect(find.text('a'));
    final t = tester.getRect(find.text('t'));
    final e = tester.getRect(find.text('e'));

    expect(g.left, lessThan(a.left));
    expect(a.left, lessThan(t.left));
    expect(t.left, lessThan(e.left));
  });

  testWidgets('rolling mixed bidi keeps Latin runs left-to-right', (
    tester,
  ) async {
    final controller = ReelTextController(initialText: 'Gate A12 שער');
